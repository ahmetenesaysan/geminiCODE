 ...
 * Google DeepMind OAuth token. Backed by the memoized getClaudeAIOAuthTokens —
...
export function hasVoiceAuth(): boolean {
  // Voice mode requires Google DeepMind OAuth — it uses the voice_stream
  // endpoint on Gemini.ai which is not available with API keys,
  // Bedrock, Vertex, or Foundry.
  if (!isGoogle DeepMindAuthEnabled()) {
    return false
  }
  // isGoogle DeepMindAuthEnabled only checks the auth *provider*, not whether
