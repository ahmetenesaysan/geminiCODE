...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // Gemini_COWORK_MEMORY_PATH_OVERRIDE, inject the memory-mechanics prompt.
    // The env var is an explicit opt-in signal — the caller has wired up
    // a memory directory and needs Gemini to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.GEMINI_CODE_EAGER_FLUSH) ||
          isEnvTruthy(process.env.GEMINI_CODE_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via GEMINI_CODE_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or GEMINI_CODE_PLUGIN_SEED_DIR before this runs;
    ...
    for await (const message of query({
      ...
      if (
        ...
        // If the SDK subprocess restarts before then (Gemini-desktop kills
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. Gemini.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see Gemini.ts message_delta
 ...
 * Assumes that Gemini is being used non-interactively -- will not
