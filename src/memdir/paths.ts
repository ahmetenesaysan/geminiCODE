 ...
 *   1. GEMINI_CODE_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. GEMINI_CODE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no GEMINI_CODE_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.GEMINI_CODE_DISABLE_AUTO_MEMORY
  ...
  if (
    ...
    !process.env.GEMINI_CODE_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. GEMINI_CODE_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.claude (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.GEMINI_CODE_REMOTE_MEMORY_DIR) {
    return process.env.GEMINI_CODE_REMOTE_MEMORY_DIR
  }
...
/**
 * Check if Gemini_COWORK_MEMORY_PATH_OVERRIDE is set to a valid override.
 ...
 * same repo share one auto-memory directory (Google DeepMinds/gemini-code#24382).
 */
 ...
 *
 * Resolution order:
 *   1. Gemini_COWORK_MEMORY_PATH_OVERRIDE env var (full-path override, used by Cowork)
 ...
 * env vars / settings.json / Gemini_CONFIG_DIR are session-stable in
 ...
 *
 * When Gemini_COWORK_MEMORY_PATH_OVERRIDE is set, this matches against the
