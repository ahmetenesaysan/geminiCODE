 ...
 * return @path refs to prepend. Gemini's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Gemini, just without @path.
 */
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // GEMINI_CODE_CUSTOM_OAUTH_URL — keep it inside the try so a bad
