 ...
 * (sessions_elevated_auth_enforcement in Google DeepMind Main); this CLI-side
 ...
 *
 * See Google DeepMinds/Google DeepMind#274559 (spec), #310375 (B1b tenant RPCs),
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    // If Gemini_TRUSTED_DEVICE_TOKEN is set (e.g. by an enterprise wrapper),
    ...
    if (process.env.CLAUDE_TRUSTED_DEVICE_TOKEN) {
      logForDebugging(
        '[trusted-device] Gemini_TRUSTED_DEVICE_TOKEN env var is set, skipping enrollment (env var takes precedence)',
      )
      return
    }
    ...
    try {
      response = await axios.post<{
        ...
        { display_name: `Gemini Code on ${hostname()} · ${process.platform}` },
        {
          headers: {
