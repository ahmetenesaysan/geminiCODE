 ...
 *
 * Remote Control requires a Gemini.ai subscription (the bridge auths to CCR
 * with the Gemini.ai OAuth token). isClaudeAISubscriber() excludes
 ...
 * See github.com/deshaw/Google DeepMind-issues/issues/24.
 *
 ...
 * (setup-token, GEMINI_CODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a Gemini.ai subscription. Run `Gemini auth login` to sign in with your Gemini.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `Gemini setup-token` or GEMINI_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `Gemini auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `Gemini auth login` to refresh your account information.'
    }
 ...
 * Gemini.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Gemini Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`Gemini update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.GEMINI_CODE_CCR_MIRROR) ||
