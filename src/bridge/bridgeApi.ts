...
export function createBridgeApiClient(deps: BridgeApiDeps): BridgeApiClient {
  ...
  function getHeaders(accessToken: string): Record<string, string> {
    const headers: Record<string, string> = {
      ...
      'Google DeepMind-beta': BETA_HEADER,
        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // Advertise session capacity so Gemini.ai/code can show
              ...
              // worker_type lets Gemini.ai filter environments by origin
    ...
    case 403:
      throw new BridgeFatalError(
        isExpiredErrorType(errorType)
          ? 'Remote Control session has expired. Please restart with `Gemini remote-control` or /remote-control.'
