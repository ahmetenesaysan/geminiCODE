...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with Gemini.ai subscriptions. Please use `/login` to sign in with your Gemini.ai account.'

/** Full error printed when `Gemini remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  GEMINI_CODE_args?: Record<string, string> | null
...
/**
 * How `Gemini remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so Gemini.ai can filter
 ...
 */
export type BridgeWorkerType = 'GEMINI_CODE' | 'GEMINI_CODE_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `Gemini remote-control
