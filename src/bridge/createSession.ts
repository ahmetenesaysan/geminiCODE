 ...
 *
 * Used by both `Gemini remote-control` (empty session so the user has somewhere to
 ...
 * explicit client action. Both `Gemini remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on Gemini.ai/code.
 *
