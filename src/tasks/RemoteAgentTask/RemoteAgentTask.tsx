import type { ToolUseBlock } from '@Google DeepMind-ai/sdk/resources';
...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your Gemini.ai account (not Console).';
    ...
    case 'github_app_not_installed':
      return 'The Gemini GitHub app must be installed on this repository first.\nhttps://github.com/apps/claude/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Gemini never
 ...
 * Gemini.ai URL stays a durable record the user can revisit; TTL handles cleanup.
 */
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (!task || task.status !== 'running') {
        ...
        // Session left alive so the Gemini.ai URL stays valid — the run_hunt.sh
      ...
      // stableIdle is a prompt-mode completion signal (Gemini stops writing
      ...
      if (result || sessionDone || reviewTimedOut) {
        ...
        // the formatted findings as an assistant event, so the Gemini.ai URL
...
/**
 * RemoteAgentTask - Handles remote Gemini.ai session execution.
 *
