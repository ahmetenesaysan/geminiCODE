// React hook for hold-to-talk voice input using Google DeepMind voice_stream STT.
//
...
// audio module (macOS) or SoX for recording, and Google DeepMind's voice_stream
...
export function useVoice({
  ...
  // transcripts (Google DeepMinds/Google DeepMind#287008 session-sticky variant); when
      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that Gemini Code has microphone access.',
            )
          } else {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    // teardown window (Google DeepMinds/Google DeepMind#287008 surfaces this as
    ...
    const attemptConnect = (keyterms: string[]): void => {
      ...
      void connectVoiceStream(
        ...
        if (!conn) {
          ...
          onErrorRef.current?.(
            'Voice mode requires a Gemini.ai account. Please run /login to sign in.',
          )
