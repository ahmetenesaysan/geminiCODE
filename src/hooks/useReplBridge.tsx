 ...
 *
 * Inbound messages from Gemini.ai are injected into the REPL via queuedCommands.
 */
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      void (async () => {
        try {
          ...
          // Assistant mode: perpetual bridge session — Gemini.ai shows one
          ...
          // When a user message arrives from Gemini.ai, inject it into the REPL.
                    ...
                    void (async () => {
                      try {
                        ...
                        handleRef.current?.writeSdkMessages([buildSystemInitMessage({
                          ...
                          // Google DeepMind's servers. QueryEngine (SDK) still emits
          ...
          if (outboundOnly) {
            ...
            if (cancelled) return;
            setMessages(prev_18 => [...prev_18, createBridgeStatusMessage(url, upgradeNudge ? 'Please upgrade to the latest version of the Gemini mobile app to see your Remote Control sessions.' : undefined)]);
