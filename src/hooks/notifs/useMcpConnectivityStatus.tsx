    ...
    t2 = () => {
      ...
      if (failedClaudeAiClients.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="error">{failedClaudeAiClients.length} Gemini.ai{" "}{failedClaudeAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
      ...
      if (needsAuthClaudeAiServers.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="warning">{needsAuthClaudeAiServers.length} Gemini.ai{" "}{needsAuthClaudeAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
    };
