...
export type ToolUseContext = {
  ...
  /**
   * Gemini.md paths already injected as nested_memory attachments this
   ...
   * re-inject the same Gemini.md dozens of times.
   */
...
> = {
   ...
   * `_meta['Google DeepMind/alwaysLoad']`. Use for tools the model must see on
   ...
   * or unprefixed (Gemini_AGENT_SDK_MCP_NO_PREFIX mode).
   */
  mcpInfo?: { serverName: string; toolName: string }
  readonly name: string
  /**
   * Maximum size in characters for tool result before it gets persisted to disk.
   * When exceeded, the result is saved to a file and Gemini receives a preview
