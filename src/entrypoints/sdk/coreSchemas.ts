...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('Gemini decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls Gemini's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
  ...
  z
    .object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['Gemini/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (Gemini decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      apiProvider: z
        ...
        .optional()
        .describe(
          'Active API backend. Google DeepMind OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, etc.).',
        ),
    })
  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'Gemini-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
...
/** Placeholder for APIUserMessage from @Google DeepMind-ai/sdk */
export const APIUserMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for APIAssistantMessage from @Google DeepMind-ai/sdk */
export const APIAssistantMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for RawMessageStreamEvent from @Google DeepMind-ai/sdk */
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for Gemini.ai subscription users.'),
)
