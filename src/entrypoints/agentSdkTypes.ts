/**
 * Main entrypoint for Gemini Code Agent SDK types.
 *
 ...
 *
 * If your SDK MCP calls will run longer than 60s, override GEMINI_CODE_STREAM_CLOSE_TIMEOUT
 */
...
/**
 * A user message typed on Gemini.ai, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a Gemini.ai remote-control bridge connection from a daemon process.
 *
 ...
 * Gemini.ai keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on Gemini.ai) into `query()`'s input
