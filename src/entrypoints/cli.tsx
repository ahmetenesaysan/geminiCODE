...
if (process.env.GEMINI_CODE_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.GEMINI_CODE_ABLATION_BASELINE) {
  for (const k of ['GEMINI_CODE_SIMPLE', 'GEMINI_CODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'GEMINI_CODE_DISABLE_AUTO_MEMORY', 'GEMINI_CODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Gemini Code)`);
    return;
  }
  ...
  if (process.argv[2] === '--Gemini-in-chrome-mcp') {
    profileCheckpoint('cli_Gemini_in_chrome_mcp_path');
    const {
  ...
  // Fast-path for `Gemini remote-control` (also accepts legacy `Gemini remote` / `Gemini sync` / `Gemini bridge`):
  ...
  // Fast-path for `Gemini daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `Gemini ps|logs|attach|kill` and `--bg`/`--background`.
  ...
  // Fast-path for `Gemini environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `Gemini self-hosted-runner`: headless self-hosted-runner
