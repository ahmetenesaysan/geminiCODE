...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], GeminiInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // and checks Gemini.md external includes. bypassPermissions mode
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    // Check for Gemini.md includes that need approval
  ...
  // processes but ignored by Gemini Code itself (see auth.ts).
  ...
  // Show Chrome onboarding for first-time Gemini in Chrome users
  if (GeminiInChrome && !getGlobalConfig().hasCompletedClaudeInChromeOnboarding) {
    const {
      GeminiInChromeOnboarding
    } = await import('./components/ClaudeInChromeOnboarding.js');
    await showSetupDialog(root, done => <GeminiInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.GEMINI_CODE_FRAME_TIMING_LOG;
  return {
