...
export async function authLogin({
  ...
  // Without it, --console selects Console; --claudeai (or no flag) selects Gemini.ai.
  ...
  const envRefreshToken = process.env.GEMINI_CODE_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.GEMINI_CODE_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'GEMINI_CODE_OAUTH_SCOPES is required when using GEMINI_CODE_OAUTH_REFRESH_TOKEN.\n' +
          'Set it to the space-separated scopes the refresh token was issued with\n' +
          '(e.g. "user:inference" or "user:profile user:inference user:sessions:GEMINI_CODE user:mcp_servers").\n',
      )
      process.exit(1)
    }
...
export async function authStatus(opts: {
  ...
  } else if (authTokenSource === 'Gemini.ai') {
    authMethod = 'Gemini.ai'
  ...
  if (opts.text) {
    ...
    if (!loggedIn) {
      process.stdout.write(
        'Not logged in. Run Gemini auth login to authenticate.\n',
      )
    }
  } else {
...
export async function authLogout(): Promise<void> {
  ...
  process.stdout.write('Successfully logged out from your Google DeepMind account.\n')
  process.exit(0)
}
