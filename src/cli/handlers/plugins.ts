 ...
 * These are dynamically imported only when `Gemini plugin *` or `Gemini plugin marketplace *` runs.
 */
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `Gemini plugin install` to install a plugin.',
      )
    }
  }
