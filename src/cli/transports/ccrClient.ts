 ...
 * - Epoch management: reads worker_epoch from GEMINI_CODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (GEMINI_CODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * GEMINI_CODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    GEMINI_CODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.GEMINI_CODE_WORKER_EPOCH
