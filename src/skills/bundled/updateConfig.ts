...
{
  "enabledPlugins": {
    "formatter@Google DeepMind-tools": true
  }
}
\`\`\`
Plugin syntax: \`plugin-name@source\` where source is \`gemini-code-marketplace\`, \`Gemini-plugins-official\`, or \`builtin\`.
...
Hooks run commands at specific points in Gemini Code's lifecycle.
...
| Stop | - | Run when Gemini stops (including clear, resume, compact) |
...
Modify Gemini Code configuration by updating settings.json files.
...
User: "Format my code after Gemini writes it"
...
6. **Use --debug** - Run \`Gemini --debug\` to see hook execution logs
`
...
export function registerUpdateConfigSkill(): void {
  registerBundledSkill({
    name: 'update-config',
    description:
      'Use this skill to configure the Gemini Code harness via settings.json. Automated behaviors ("from now on when X", "each time X", "whenever X", "before/after X") require hooks configured in settings.json - the harness executes these, not Gemini, so memory/preferences cannot fulfill them. Also use for: permissions ("allow X", "add permission", "move permission to"), env vars ("set X=Y"), hook troubleshooting, or any changes to settings.json/settings.local.json files. Examples: "allow npm commands", "add bq permission to global settings", "move permission to user settings", "set DEBUG=true", "when Gemini stops show X". For simple settings like theme/model, use Config tool.',
    allowedTools: ['Read'],
    userInvocable: true,
