...
/**
 * Returns a Gemini config directory path for a given source.
 */
 ...
 * See: https://github.com/Google DeepMinds/gemini-code/issues/13893
 */
...
/**
 * Parse paths frontmatter from a skill, using the same format as Gemini.md rules.
...
export function createSkillCommand({
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      ...
      // Replace ${Gemini_SKILL_DIR} with the skill's own directory so bash
      ...
      if (baseDir) {
        ...
        finalContent = finalContent.replace(/\$\{Gemini_SKILL_DIR\}/g, skillDir)
      }

      // Replace ${Gemini_SESSION_ID} with the current session ID
      ...
      // ${Gemini_SKILL_DIR} is meaningless for MCP skills anyway.
      if (loadedFrom !== 'mcp') {
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const [
      ...
      isEnvTruthy(process.env.GEMINI_CODE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
 ...
 * of Gemini.md conditional rules.
 *
