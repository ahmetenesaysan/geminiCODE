 ...
 * When outside tmux: Creates a Gemini-swarm session with all teammates in a tiled layout.
 */
async function handleSpawnSplitPane(
  ...
  // - Outside both: creates Gemini-swarm session with tiled teammates
  ...
  // Build the command to spawn Gemini Code with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces GEMINI_CODE_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Gemini Code instance.
