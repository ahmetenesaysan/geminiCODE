...
export type PermissionHandlerOptions = {
  ...
  scope?: 'Gemini-folder' | 'global-Gemini-folder'
}
...
function handleAcceptSession(
  ...
  // For Gemini-folder scope, grant session-level access to all .claude/ files
  if (
    options?.scope === 'Gemini-folder' ||
    options?.scope === 'global-Gemini-folder'
  ) {
    const pattern =
      options.scope === 'global-Gemini-folder'
        ? GLOBAL_Gemini_FOLDER_PERMISSION_PATTERN
