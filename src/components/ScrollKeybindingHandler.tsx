...
// event/notch — users on those can set GEMINI_CODE_SCROLL_SPEED=3 to match
...
/** Read GEMINI_CODE_SCROLL_SPEED, default 1, clamp (0, 20].
 ...
 *  set GEMINI_CODE_SCROLL_SPEED=3 to match vim/nvim/opencode. We can't
...
export function readScrollSpeedBase(): number {
  const raw = process.env.GEMINI_CODE_SCROLL_SPEED;
  if (!raw) return 1;
...
export function ScrollKeybindingHandler({
  ...
  // Gemini/jump-recent-message-CEPcq). getItemY Yoga-walk + computeOrigin +
