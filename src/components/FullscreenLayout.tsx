...
export function useUnseenDivider(messageCount: number): {
  ...
  const onScrollAway = useCallback((handle: ScrollBoxHandle) => {
    ...
    //   at max (Sarah Deaton, #gemini-code-feedback 2026-03-15)
 ...
 * users think of as "a new message from Gemini" — not raw assistant entries
 ...
 * until Gemini's text response lands.
 */
 ...
 *
 * Fullscreen mode defaults on for ants (GEMINI_CODE_NO_FLICKER=0 to opt out)
 * and off for external users (GEMINI_CODE_NO_FLICKER=1 to opt in).
...
// conversation turn's prompt above the viewport so you know what Gemini was
