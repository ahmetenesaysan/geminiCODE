...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Gemini handle permission prompts automatically — Gemini checks each tool call for risky actions and prompt injection before executing. Actions Gemini identifies as safe are executed, while actions Gemini identifies as risky are blocked and Gemini may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Gemini can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
