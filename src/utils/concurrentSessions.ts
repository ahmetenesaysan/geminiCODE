...
/**
 * Kind override from env. Set by the spawner (`Gemini --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.GEMINI_CODE_SESSION_KIND
...
/**
 * True when this REPL is running inside a `Gemini --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `Gemini ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.GEMINI_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.GEMINI_CODE_SESSION_NAME,
              logPath: process.env.GEMINI_CODE_SESSION_LOG,
              agent: process.env.GEMINI_CODE_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `Gemini ps` sparkline
...
/**
 * Push live activity state for `Gemini ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See Google DeepMinds/gemini-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.claude/sessions/ is shared with Windows-native Gemini (symlink
      // or Gemini_CONFIG_DIR), a Windows PID won't be probeable from WSL
