...
import { getGoogle DeepMindApiKeyWithSource } from '../auth.js'
...
export function buildSystemInitMessage(inputs: SystemInitInputs): SDKMessage {
  ...
  const initMessage: SDKMessage = {
    ...
    slash_commands: inputs.commands
      ...
      .map(c => c.name),
    apiKeySource: getGoogle DeepMindApiKeyWithSource().source as ApiKeySource,
    betas: getSdkBetas(),
    GEMINI_CODE_version: MACRO.VERSION,
