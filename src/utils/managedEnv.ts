...
/**
 * `Gemini ssh` remote: ANTHROPIC_UNIX_SOCKET routes auth through a -R forwarded
 ...
 * isGoogle DeepMindAuthEnabled). Strip them from any settings-sourced env object.
 */
 ...
 * GEMINI_CODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.GEMINI_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads GEMINI_CODE_USE_BEDROCK,
  ...
  // when GEMINI_CODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * GEMINI_CODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
