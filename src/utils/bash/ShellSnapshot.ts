...
/**
 * Generates Gemini Code specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Gemini's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Gemini Code content
 ...
 *
 * This function is a critical part of Gemini CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Gemini CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Gemini Code defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.GEMINI_CODE_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Gemini home: ${getClaudeConfigHomeDir()}`)
