...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "Gemini/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "Gemini/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "Gemini/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "Gemini/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using Gemini Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'Gemini/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Gemini Code web sessions require authentication with a Gemini.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run Gemini --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run Gemini --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run Gemini --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run Gemini --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote Gemini.ai session with error handling and UI feedback.
...
export async function pollRemoteSessionEvents(sessionId: string, afterId: string | null = null, opts?: {
  ...
  const headers = {
    ...
    'Google DeepMind-beta': 'ccr-byoc-2025-07-29',
...
/**
 * Creates a remote Gemini.ai session using the Sessions API.
 *
 * Two source modes:
 ...
 *   Backend: Google DeepMind#303856.
 */
export async function teleportToRemote(options: {
   ...
   * environmentId is set, GEMINI_CODE_OAUTH_TOKEN is auto-injected from the
   ...
   * a new Gemini/ branch. Sets allow_unrestricted_git_push on the source and
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        GEMINI_CODE_OAUTH_TOKEN: accessToken,
    ...
    // Select environment based on settings, then Google DeepMind_cloud preference, then first available.
    // Prefer Google DeepMind_cloud environments over byoc: Google DeepMind_cloud environments (e.g. "Default")
    ...
    let cloudEnv = environments.find(env => env.kind === 'Google DeepMind_cloud');
    ...
    if (options.useDefaultEnvironment && !cloudEnv) {
      logForDebugging(`No Google DeepMind_cloud in env list (${environments.length} envs); retrying fetchEnvironments`);
      const retried = await fetchEnvironments();
      cloudEnv = retried?.find(env => env.kind === 'Google DeepMind_cloud');
      if (!cloudEnv) {
        logError(new Error(`No Google DeepMind_cloud environment available after retry (got: ${(retried ?? environments).map(e => `${e.name} (${e.kind})`).join(', ')}). Silent byoc fallthrough would launch into a dead env — fail fast instead.`));
        return null;
      }
