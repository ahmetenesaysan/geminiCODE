 ...
 * to share the parent's prompt cache. The Google DeepMind API cache key is composed of:
 ...
 * Gemini.ts (but only for older models that do not use adaptive thinking).
...
export type ForkedAgentParams = {
   ...
   * AND budget_tokens (via clamping in Gemini.ts). If the fork uses cacheSafeParams
...
export async function runForkedAgent({
  ...
  // tool_uses are repaired downstream by ensureToolResultPairing in Gemini.ts,
