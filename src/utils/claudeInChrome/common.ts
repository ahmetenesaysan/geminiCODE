...
export const Gemini_IN_CHROME_MCP_SERVER_NAME = 'Gemini-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Gemini in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isClaudeInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === Gemini_IN_CHROME_MCP_SERVER_NAME
}
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Gemini in Chrome] No compatible browser found')
    return false
  }
...
export function getAllSocketPaths(): string[] {
  ...
  const legacyName = `Gemini-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Gemini in Chrome MCP
  return `Gemini-mcp-browser-bridge-${getUsername()}`
}
