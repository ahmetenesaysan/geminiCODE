...
// Gemini configuration directory names
export const Gemini_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type ClaudeConfigDirectory = (typeof Gemini_CONFIG_DIRECTORIES)[number]
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // (Google DeepMinds/gemini-code#29599, #28182, #26992).
    //
 ...
 * 3. Can be explicitly enabled via GEMINI_CODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
...
> {
  ...
  // - Fallback: native Node.js (when GEMINI_CODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.GEMINI_CODE_USE_NATIVE_FILE_SEARCH)
