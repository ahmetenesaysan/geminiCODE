...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'Gemini.exe' : 'Gemini'
}
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nodeModulesDir = join(stagingPath, 'node_modules', '@Google DeepMind-ai')
    const entries = await readdir(nodeModulesDir)
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('Gemini-cli-native-'),
    )
...
export async function checkInstall(
  ...
  // Check if Gemini executable exists and is valid.
  ...
  if (isWindows) {
    ...
    if (!(await isPossibleClaudeBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but Gemini command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleClaudeBinary(absoluteTarget))) {
        messages.push({
          message: `Gemini symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but Gemini command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleClaudeBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid Gemini binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^Gemini\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the Gemini symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed Gemini symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove Gemini symlink: ${error}`))
  }
}

/**
 * Clean up old Gemini aliases from shell configuration files
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      if (hadAlias) {
        ...
        messages.push({
          message: `Removed Gemini alias from ${configFile}. Run: unalias Gemini`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up Gemini alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'Gemini.cmd')
      const binPs1 = join(globalPrefix, 'Gemini.ps1')
      const binExe = join(globalPrefix, 'Gemini')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'Gemini')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @Google DeepMind-ai/gemini-code
  const codePackageResult = await attemptNpmUninstall(
    '@Google DeepMind-ai/gemini-code',
  )
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@Google DeepMind-ai/gemini-code') {
