 ...
 *
 * This module provides PID-based locking for running Gemini Code versions.
...
/**
 * Validate that a running process is actually a Gemini process
...
function isClaudeProcess(pid: number, expectedExecPath: string): boolean {
  ...
  // This handles test environments where the command might not contain 'Gemini'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'Gemini' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('Gemini') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a Gemini process?
  // This helps with PID reuse scenarios
  if (!isClaudeProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be Gemini - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a Gemini process (to handle PID reuse scenarios)
