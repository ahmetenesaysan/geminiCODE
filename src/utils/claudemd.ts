 ...
 *
 * 1. Managed memory (eg. /etc/gemini-code/CLAUDE.md) - Global instructions for all users
 * 2. User memory (~/.claude/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (Gemini.md, .claude/CLAUDE.md, and .claude/rules/*.md in project roots) - Instructions checked into the codebase
 * 4. Local memory (Gemini.local.md in project roots) - Private project-specific instructions
 *
 ...
 * - Gemini.md, .claude/CLAUDE.md, and all .md files in .claude/rules/ are checked in each directory for Project memory
 *
...
function handleMemoryFileReadError(error: unknown, filePath: string): void {
  ...
  if (code === 'EACCES') {
    ...
    logEvent('tengu_Gemini_md_permission_error', {
      is_access_error: 1,
...
/**
 * Checks whether a Gemini.md file path is excluded by the claudeMdExcludes setting.
...
export async function processMdRules({
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('EACCES')) {
      logEvent('tengu_Gemini_rules_md_permission_error', {
        is_access_error: 1,
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // .claude/worktrees/<name>/ from `Gemini -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like Gemini.md and .claude/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. Gemini.local.md is gitignored so it only
    // exists in the main repo and is still loaded.
    // See: https://github.com/Google DeepMinds/gemini-code/issues/29599
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading Gemini.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'Gemini.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        const dotClaudePath = join(dir, '.claude', 'Gemini.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
      ...
      // Try reading Gemini.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'Gemini.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process Gemini.md from additional directories (--add-dir) if env var is enabled
    // This is controlled by GEMINI_CODE_ADDITIONAL_DIRECTORIES_Gemini_MD and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.GEMINI_CODE_ADDITIONAL_DIRECTORIES_Gemini_MD)) {
      const additionalDirs = getAdditionalDirectoriesForClaudeMd()
      for (const dir of additionalDirs) {
        // Try reading Gemini.md from the additional directory
        const projectPath = join(dir, 'Gemini.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
    ...
    // memory system, not "instructions" in the Gemini.md/rules sense.
 ...
 * Loads Gemini.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (Gemini.md and .claude/CLAUDE.md)
  ...
  // Process local memory file (Gemini.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'Gemini.local.md')
    result.push(
...
/**
 * Check if a file path is a memory file (Gemini.md, Gemini.local.md, or .claude/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // Gemini.md or Gemini.local.md anywhere
  if (name === 'Gemini.md' || name === 'Gemini.local.md') {
    return true
  }
