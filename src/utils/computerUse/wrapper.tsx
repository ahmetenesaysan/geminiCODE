...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another Gemini session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'Gemini is using your computer · press Esc to stop' : 'Gemini is using your computer · press Ctrl+C to stop',
...
export function getComputerUseMCPToolOverrides(toolName: string): ComputerUseMCPToolOverrides {
  const call: CallOverride = async (args, context: ToolUseContext) => {
    ...
    // MCP content blocks → Google DeepMind API blocks. CU only produces text and
