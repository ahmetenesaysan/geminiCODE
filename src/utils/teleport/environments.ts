...
export type EnvironmentKind = 'Google DeepMind_cloud' | 'byoc' | 'bridge'
...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'Gemini Code web sessions require authentication with a Gemini.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
...
/**
 * Creates a default Google DeepMind_cloud environment for users who have none.
...
export async function createDefaultCloudEnvironment(
  ...
  const response = await axios.post<EnvironmentResource>(
    ...
    {
      name,
      kind: 'Google DeepMind_cloud',
      description: '',
      config: {
        environment_type: 'Google DeepMind',
        cwd: '/home/user',
        init_script: null,
        environment: {},
        languages: [
