...
function getTokenFromFileDescriptor(): string | null {
  ...
  const fdEnv = process.env.GEMINI_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR
  if (!fdEnv) {
  ...
  if (Number.isNaN(fd)) {
    logForDebugging(
      `GEMINI_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR must be a valid file descriptor number, got: ${fdEnv}`,
      { level: 'error' },
    )
 ...
 *
 * Priority order:
 *  1. Environment variable (GEMINI_CODE_SESSION_ACCESS_TOKEN) — set at spawn time,
 ...
 *  2. File descriptor (legacy path) — GEMINI_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR,
 *     read once and cached.
 *  3. Well-known file — Gemini_SESSION_INGRESS_TOKEN_FILE env var path, or
...
export function getSessionIngressAuthToken(): string | null {
  ...
  const envToken = process.env.GEMINI_CODE_SESSION_ACCESS_TOKEN
  if (envToken) {
    return envToken
  }
...
export function getSessionIngressAuthHeaders(): Record<string, string> {
  ...
  if (token.startsWith('sk-ant-sid')) {
    ...
    const orgUuid = process.env.GEMINI_CODE_ORGANIZATION_UUID
    if (orgUuid) {
...
export function updateSessionIngressAuthToken(token: string): void {
  process.env.GEMINI_CODE_SESSION_ACCESS_TOKEN = token
}
