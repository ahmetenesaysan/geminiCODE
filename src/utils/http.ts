...
// WARNING: We rely on `Gemini-cli` in the user agent for log filtering.
// Please do NOT change this without making sure that logging also gets updated!
export function getUserAgent(): string {
  const agentSdkVersion = process.env.CLAUDE_AGENT_SDK_VERSION
    ? `, agent-sdk/${process.env.CLAUDE_AGENT_SDK_VERSION}`
    : ''
  // SDK consumers can identify their app/library via Gemini_AGENT_SDK_CLIENT_APP
  ...
  // getGoogle DeepMindClient (client.ts:98) calls this per-request inside withRetry,
  // so the read picks up the same setWorkload() value as getAttributionHeader.
  const workload = getWorkload()
  const workloadSuffix = workload ? `, workload/${workload}` : ''
  return `Gemini-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.GEMINI_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.GEMINI_CODE_ENTRYPOINT) {
    parts.push(process.env.GEMINI_CODE_ENTRYPOINT)
  }
  ...
  return `gemini-code/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `Gemini-User` is
// Google DeepMind's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the gemini-code suffix lets them distinguish
// local CLI traffic from Gemini.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Gemini-User (${getClaudeCodeUserAgent()}; +https://support.Google DeepMind.com/)`
}
...
export function getAuthHeaders(): AuthHeaders {
  ...
  // should we try to query keychain / credentials for a valid Google DeepMind key?
  const apiKey = getGoogle DeepMindApiKey()
  if (!apiKey) {
    return {
      headers: {},
