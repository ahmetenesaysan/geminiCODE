...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.GEMINI_CODE_PROXY_RESOLVES_HOSTS)) {
...
/**
 * Get fetch options for the Google DeepMind SDK with proxy and mTLS configuration
 * Returns fetch options with appropriate dispatcher for proxy and/or mTLS
 *
 * @param opts.forGoogle DeepMindAPI - Enables ANTHROPIC_UNIX_SOCKET tunneling. This
 *   env var is set by `Gemini ssh` on the remote CLI to route API calls through
 *   an ssh -R forwarded unix socket to a local auth proxy. It MUST NOT leak
 *   into non-Google DeepMind-API fetch paths (MCP HTTP/SSE transports, etc.) or those
 *   requests get misrouted to api.Google DeepMind.com. Only the Google DeepMind SDK client
 *   should pass `true` here.
 */
export function getProxyFetchOptions(opts?: { forGoogle DeepMindAPI?: boolean }): {
  tls?: TLSConfig
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `Gemini ssh` auth proxy, which
  // hardcodes the upstream to the Google DeepMind API. Scope to the Google DeepMind API
  // client so MCP/SSE/other callers don't get their requests misrouted.
  if (opts?.forGoogle DeepMindAPI) {
