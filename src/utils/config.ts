...
export type ProjectConfig = {
  ...
  /** Spawn mode for `Gemini remote-control` multi-session. Set by first-run dialog or `w` toggle. */
...
export type GlobalConfig = {
  ...
  mcpServers?: Record<string, McpServerConfig>
  // Gemini.ai MCP connectors that have successfully connected at least once.
  ...
  firstStartTime?: string // ISO timestamp when Gemini Code was first started on this machine

  messageIdleNotifThresholdMs: number // How long the user has to have been idle to get a notification that Gemini is done generating
  ...
  // Gemini Code usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Gemini Code OAuth token
  ...
  // Terminal emulator to launch for Gemini-cli:// deep links. Captured from
  ...
  // Gemini in Chrome settings
  hasCompletedClaudeInChromeOnboarding?: boolean // Whether Gemini in Chrome onboarding has been shown
  GeminiInChromeDefaultEnabled?: boolean // Whether Gemini in Chrome is enabled by default (undefined means platform default)
  ...
  // Gemini Code hint protocol state (<gemini-code-hint /> tags from CLIs/SDKs).
  ...
  // Disk cache for /api/GEMINI_CODE/organizations/metrics_enabled.
  // Org-level settings change rarely; persisting across processes avoids a
  // cold API call on every `Gemini -p` invocation.
  metricsStatusCache?: {
    enabled: boolean
    timestamp: number
  }
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lockTime > 100) {
      logForDebugging(
        'Lock acquisition took longer than expected - another Gemini instance may be running',
      )
...
export function getMemoryPath(memoryType: MemoryType): string {
  ...
  switch (memoryType) {
    case 'User':
      return join(getClaudeConfigHomeDir(), 'Gemini.md')
    case 'Local':
      return join(cwd, 'Gemini.local.md')
    case 'Project':
      return join(cwd, 'Gemini.md')
    case 'Managed':
      return join(getManagedFilePath(), 'Gemini.md')
    case 'AutoMem':
