...
> {
  ...
  const packagesToCheck = ['@Google DeepMind-ai/gemini-code']
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'Gemini')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'Gemini')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: Gemini install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run Gemini install to update configuration',
      })
    }
  }
  ...
  if (type === 'npm-local') {
    // Check if Gemini is already accessible via PATH
    const whichResult = await which('Gemini')
    const GeminiInPath = !!whichResult

    // Only show warning if Gemini is NOT in PATH AND no valid alias exists
    if (!GeminiInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias Gemini="~/.claude/local/claude"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias Gemini="~/.claude/local/claude"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @Google DeepMind-ai/gemini-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@Google DeepMind-ai/gemini-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `Gemini install` for native installation',
      })
    }
  }
