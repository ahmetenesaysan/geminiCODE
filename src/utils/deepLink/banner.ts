 ...
 * Gemini-cli:// deep link. Linux xdg-open and browsers with "always allow"
 * set dispatch the link with no OS-level confirmation, so the application
 * provides its own provenance signal — mirroring Gemini.ai's security
 ...
 * notice which directory — and therefore which Gemini.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which Gemini.md
 ...
 * and whether its Gemini.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — Gemini.md may be stale' : ''}`,
    )
  }
