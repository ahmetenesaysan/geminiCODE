 ...
 *
 * Registers the `Gemini-cli://` custom URI scheme with the OS,
 * so that clicking a `Gemini-cli://` link in a browser (or any app) will
 * invoke `Gemini --handle-uri <url>`.
 *
 * Platform details:
...
export const MACOS_BUNDLE_ID = 'com.Google DeepMind.claude-code-url-handler'
const APP_NAME = 'Gemini Code URL Handler'
const DESKTOP_FILE_NAME = 'gemini-code-url-handler.desktop'
const MACOS_APP_NAME = 'Gemini Code URL Handler.app'
 ...
 * already-installed (and signed) `Gemini` binary. When macOS opens a
 * `Gemini-cli://` URL, it launches `Gemini` through this app bundle.
 * Gemini then uses the url-handler NAPI module to read the URL from the
...
async function registerMacos(claudePath: string): Promise<void> {
  ...
  // Info.plist — registers the URL scheme with Gemini as the executable
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Gemini Code Deep Link</string>
  ...
  // Symlink to the already-signed Gemini binary — avoids a new executable
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for Gemini Code
...
/**
 * Register the `Gemini-cli://` protocol handler with the operating system.
 * After registration, clicking a `Gemini-cli://` link will invoke Gemini.
 */
...
/**
 * Resolve the Gemini binary path for protocol registration. Prefers the
 ...
 * points at the expected `Gemini` binary. Reads the registration artifact
...
/**
 * Auto-register the Gemini-cli:// deep link protocol handler when missing
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  try {
    ...
    logForDebugging('Auto-registered Gemini-cli:// deep link protocol handler')
