 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches Gemini Code
...
/**
 * Launch Gemini Code in the detected terminal emulator.
 *
 ...
 * chdir(cwd) and execvp(Gemini, argv). Spaces/quotes/metacharacters in
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after Gemini exits.
 ...
 * *child process* (Gemini.exe) uses CommandLineToArgvW, where a trailing
