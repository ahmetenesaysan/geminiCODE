 ...
 * Used for managing Gemini aliases and PATH entries
 */
...
export const Gemini_ALIAS_REGEX = /^\s*alias\s+Gemini\s*=/
...
/**
 * Filter out installer-created Gemini aliases from an array of lines
...
export function filterClaudeAliases(lines: string[]): {
  ...
  const filtered = lines.filter(line => {
    // Check if this is a Gemini alias
    if (Gemini_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+Gemini\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+Gemini\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a Gemini alias exists in any shell config file
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (Gemini_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+Gemini=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a Gemini alias exists and points to a valid executable
