/**
 * Marketplace manager for Gemini Code plugins
 *
...
export function getDeclaredMarketplaces(): Record<string, DeclaredMarketplace> {
  ...
  // An explicit extraKnownMarketplaces entry for Gemini-plugins-official
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Gemini Code bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via GEMINI_CODE_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.GEMINI_CODE_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via GEMINI_CODE_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via GEMINI_CODE_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set GEMINI_CODE_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'gemini-code-Plugin-Manager',
  }
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: Gemini plugin disable <plugin>@${name}`,
    )
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Gemini Code version. Run 'Gemini marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `Gemini plugin marketplace update` (no name arg).
  ...
  try {
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: Gemini plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.GEMINI_CODE_REMOTE)) {
      ...
      try {
        ...
        const reason =
          name === 'gemini-code-plugins'
            ? `We've deprecated "gemini-code-plugins" in favor of "Gemini-plugins-official".`
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: Gemini plugin marketplace remove "${name}"`,
        )
      }
