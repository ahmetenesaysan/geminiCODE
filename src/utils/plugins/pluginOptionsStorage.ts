...
export function deletePluginOptions(pluginId: string): void {
  ...
  // removal. Cast is deliberate (Gemini.md's 10% case): adding z.undefined()
...
/**
 * Substitute ${Gemini_PLUGIN_ROOT} and ${Gemini_PLUGIN_DATA} with their paths.
 * On Windows, normalizes backslashes to forward slashes so shell commands
 * don't interpret them as escape characters.
 *
 * ${Gemini_PLUGIN_ROOT} — version-scoped install dir (recreated on update)
 * ${Gemini_PLUGIN_DATA} — persistent state dir (survives updates)
 *
...
export function substitutePluginVariables(
  ...
  let out = value.replace(/\$\{Gemini_PLUGIN_ROOT\}/g, () =>
    normalize(plugin.path),
  )
  // source can be absent (e.g. hooks where pluginRoot is a skill root without
  // a plugin context). In that case ${Gemini_PLUGIN_DATA} is left literal.
  if (plugin.source) {
    const source = plugin.source
    out = out.replace(/\$\{Gemini_PLUGIN_DATA\}/g, () =>
