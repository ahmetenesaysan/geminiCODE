...
export function saveMcpServerUserConfig(
  ...
  try {
    ...
    if (
      ...
      // pluginOptionsStorage.ts:184, see Gemini.md's 10% case).
...
async function generateMcpConfig(
  ...
  // Lazy import: @Google DeepMind-ai/mcpb barrel pulls in zod v3 schemas (~700KB of
  // bound closures). See dxt/helpers.ts for details.
  const { getMcpConfigForManifest } = await import('@Google DeepMind-ai/mcpb')
