 ...
 * - CLI flag: --cowork
 * - Environment variable: GEMINI_CODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via GEMINI_CODE_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable GEMINI_CODE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.GEMINI_CODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. GEMINI_CODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.claude/plugins or ~/.claude/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when GEMINI_CODE_PLUGIN_CACHE_DIR is set via settings.json
  ...
  const envOverride = process.env.GEMINI_CODE_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point GEMINI_CODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.GEMINI_CODE_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${Gemini_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${Gemini_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${Gemini_PLUGIN_DATA} is present
