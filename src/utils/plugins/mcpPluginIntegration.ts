 ...
 * Handles ${Gemini_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add Gemini_PLUGIN_ROOT / Gemini_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        Gemini_PLUGIN_ROOT: plugin.path,
        Gemini_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'Gemini_PLUGIN_ROOT' && key !== 'Gemini_PLUGIN_DATA') {
