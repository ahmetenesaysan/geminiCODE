 ...
 * @Google DeepMind-ai/sandbox-runtime's shellquote.quote() which would otherwise
  ...
  return {
    ...
    async buildExecCommand(
      ...
      const cwdFilePath =
        opts.useSandbox && opts.sandboxTmpDir
          ? posixJoin(opts.sandboxTmpDir, `Gemini-pwd-ps-${opts.id}`)
          : join(tmpdir(), `Gemini-pwd-ps-${opts.id}`)
    ...
    async getEnvironmentOverrides(): Promise<Record<string, string>> {
      ...
      if (currentSandboxTmpDir) {
        ...
        env.GEMINI_CODE_TMPDIR = currentSandboxTmpDir
      }
      return env
    },
  }
