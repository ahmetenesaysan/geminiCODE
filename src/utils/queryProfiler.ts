 ...
 * pipeline from user input to first token arrival. Enable by setting GEMINI_CODE_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating Google DeepMind client
...
const ENABLED = isEnvTruthy(process.env.GEMINI_CODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set GEMINI_CODE_PROFILE_QUERY=1)'
  }
