...
export async function getSessionEnvironmentScript(): Promise<string | null> {
  ...
  // Check for Gemini_ENV_FILE passed from parent process (e.g., HFI trajectory runner)
  ...
  if (envFile) {
    try {
      ...
      if (envScript) {
        ...
        logForDebugging(
          `Session environment loaded from Gemini_ENV_FILE: ${envFile} (${envScript.length} chars)`,
        )
      }
    } catch (e: unknown) {
      const code = getErrnoCode(e)
      if (code !== 'ENOENT') {
        logForDebugging(`Failed to read Gemini_ENV_FILE: ${errorMessage(e)}`)
      }
    }
  }
