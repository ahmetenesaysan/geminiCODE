...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    const Google DeepMindBaseUrl = process.env.ANTHROPIC_BASE_URL;
    if (Google DeepMindBaseUrl) {
      properties.push({
        label: 'Google DeepMind base URL',
        value: Google DeepMindBaseUrl
      });
    }
    ...
    if (isEnvTruthy(process.env.GEMINI_CODE_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.GEMINI_CODE_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.GEMINI_CODE_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.GEMINI_CODE_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.GEMINI_CODE_CLIENT_KEY
      });
    }
  }
  return properties;
