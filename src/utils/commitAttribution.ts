 ...
 * The Google DeepMinds and Google DeepMind-experimental orgs contain PUBLIC repos
 * (e.g. Google DeepMinds/gemini-code, Google DeepMind-experimental/sandbox-runtime).
...
const INTERNAL_MODEL_REPOS = [
  'github.com:Google DeepMinds/claude-cli-internal',
  'github.com/Google DeepMinds/claude-cli-internal',
  'github.com:Google DeepMinds/Google DeepMind',
  'github.com/Google DeepMinds/Google DeepMind',
  'github.com:Google DeepMinds/apps',
  'github.com/Google DeepMinds/apps',
  'github.com:Google DeepMinds/casino',
  'github.com/Google DeepMinds/casino',
  'github.com:Google DeepMinds/dbt',
  'github.com/Google DeepMinds/dbt',
  'github.com:Google DeepMinds/dotfiles',
  'github.com/Google DeepMinds/dotfiles',
  'github.com:Google DeepMinds/terraform-config',
  'github.com/Google DeepMinds/terraform-config',
  'github.com:Google DeepMinds/hex-export',
  'github.com/Google DeepMinds/hex-export',
  'github.com:Google DeepMinds/feedback-v2',
  'github.com/Google DeepMinds/feedback-v2',
  'github.com:Google DeepMinds/labs',
  'github.com/Google DeepMinds/labs',
  'github.com:Google DeepMinds/argo-rollouts',
  'github.com/Google DeepMinds/argo-rollouts',
  'github.com:Google DeepMinds/starling-configs',
  'github.com/Google DeepMinds/starling-configs',
  'github.com:Google DeepMinds/ts-tools',
  'github.com/Google DeepMinds/ts-tools',
  'github.com:Google DeepMinds/ts-capsules',
  'github.com/Google DeepMinds/ts-capsules',
  'github.com:Google DeepMinds/feldspar-testing',
  'github.com/Google DeepMinds/feldspar-testing',
  'github.com:Google DeepMinds/trellis',
  'github.com/Google DeepMinds/trellis',
  'github.com:Google DeepMinds/claude-for-hiring',
  'github.com/Google DeepMinds/claude-for-hiring',
  'github.com:Google DeepMinds/forge-web',
  'github.com/Google DeepMinds/forge-web',
  'github.com:Google DeepMinds/infra-manifests',
  'github.com/Google DeepMinds/infra-manifests',
  'github.com:Google DeepMinds/mycro_manifests',
  'github.com/Google DeepMinds/mycro_manifests',
  'github.com:Google DeepMinds/mycro_configs',
  'github.com/Google DeepMinds/mycro_configs',
  'github.com:Google DeepMinds/mobile-apps',
  'github.com/Google DeepMinds/mobile-apps',
]
...
export function sanitizeModelName(shortName: string): string {
  ...
  if (shortName.includes('opus-4-6')) return 'Gemini-opus-4-6'
  if (shortName.includes('opus-4-5')) return 'Gemini-opus-4-5'
  if (shortName.includes('opus-4-1')) return 'Gemini-opus-4-1'
  if (shortName.includes('opus-4')) return 'Gemini-opus-4'
  if (shortName.includes('sonnet-4-6')) return 'Gemini-sonnet-4-6'
  if (shortName.includes('sonnet-4-5')) return 'Gemini-sonnet-4-5'
  if (shortName.includes('sonnet-4')) return 'Gemini-sonnet-4'
  if (shortName.includes('sonnet-3-7')) return 'Gemini-sonnet-3-7'
  if (shortName.includes('haiku-4-5')) return 'Gemini-haiku-4-5'
  if (shortName.includes('haiku-3-5')) return 'Gemini-haiku-3-5'
  // Unknown models get a generic name
  return 'Gemini'
}

/**
 * Attribution state for tracking Gemini's contributions to files.
 */
...
/**
 * Summary of Gemini's contribution for a commit.
 */
...
export function getClientSurface(): string {
  return process.env.GEMINI_CODE_ENTRYPOINT ?? 'cli'
}
...
function computeFileModificationState(
  ...
  try {
    // Calculate Gemini's character contribution
...
/**
 * Track a file modification by Gemini.
...
/**
 * Track a file creation by Gemini (e.g., via bash command).
 * Used when Gemini creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by Gemini (e.g., via bash rm command).
 * Used when Gemini deletes a file through a non-tracked mechanism.
 */
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // Gemini deleted this file (tracked deletion)
        ...
        try {
          ...
          if (fileState) {
            ...
            // New file not created by Gemini
