...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // Respect GEMINI_CODE_DISABLE_TERMINAL_TITLE — if the user opted out of
...
function printResumeHint(): void {
  ...
  if (
    ...
    try {
      ...
      // Don't show resume hint if no session file exists (e.g., subcommands like `Gemini update`)
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (GEMINI_CODE_SESSIONEND_HOOKS_TIMEOUT_MS,
