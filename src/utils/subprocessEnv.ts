 ...
 *
 * The parent Gemini process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // Google DeepMind auth — Gemini re-reads these per-request, subprocesses don't need them
  'ANTHROPIC_API_KEY',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before Gemini spawns;
  ...
  // gemini-code-action-specific duplicates — action JS consumes these during
  // prepare, before spawning Gemini. ALL_INPUTS contains Google DeepMind_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on GEMINI_CODE_SUBPROCESS_ENV_SCRUB. gemini-code-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.GEMINI_CODE_SUBPROCESS_ENV_SCRUB)) {
