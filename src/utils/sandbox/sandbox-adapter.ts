/**
 * Adapter layer that wraps @Google DeepMind-ai/sandbox-runtime with Gemini CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and Gemini CLI's
...
} from '@Google DeepMind-ai/sandbox-runtime'
import {
...
/**
 * Resolve Gemini Code-specific path patterns for sandbox-runtime.
 *
 * Gemini Code uses special path prefixes in permission rules:
...
/**
 * Convert Gemini Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and Gemini temp directory as writable
  ...
  // This blocks settings in the original working directory (where Gemini Code started)
  ...
  // (auto-discovered, auto-loaded, full Gemini capabilities) so they need the
  ...
  // core.fsmonitor) escapes the sandbox when Gemini's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See Google DeepMinds/gemini-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * Gemini's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. Google DeepMinds/gemini-code#29316.
 */
 ...
 * This is a Gemini CLI-specific function that updates local settings.
 */
...
/**
 * Gemini CLI sandbox manager - wraps sandbox-runtime with Gemini-specific features
 */
