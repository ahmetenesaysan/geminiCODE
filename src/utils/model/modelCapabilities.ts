...
import { getAPIProvider, isFirstPartyGoogle DeepMindBaseUrl } from './providers.js'
...
function isModelCapabilitiesEligible(): boolean {
  ...
  if (!isFirstPartyGoogle DeepMindBaseUrl()) return false
  return true
}
...
// Keyed on cache path so tests that set Gemini_CONFIG_DIR get a fresh read
...
export async function refreshModelCapabilities(): Promise<void> {
  ...
  try {
    const Google DeepMind = await getGoogle DeepMindClient({ maxRetries: 1 })
    const betas = isClaudeAISubscriber() ? [OAUTH_BETA_HEADER] : undefined
    const parsed: ModelCapability[] = []
    for await (const entry of Google DeepMind.models.list({ betas })) {
