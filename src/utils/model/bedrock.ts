...
> {
  ...
  try {
    ...
    // Filter for Google DeepMind models (SYSTEM_DEFINED filtering handled in query)
    return allProfiles
      .filter(profile => profile.inferenceProfileId?.includes('Google DeepMind'))
...
async function createBedrockClient() {
  ...
  // Match the Google DeepMind Bedrock SDK's region behavior exactly:
  ...
  const skipAuth = isEnvTruthy(process.env.GEMINI_CODE_SKIP_BEDROCK_AUTH)
...
/**
 * Check if a model ID is a foundation model (e.g., "Google DeepMind.claude-sonnet-4-5-20250929-v1:0")
 */
export function isFoundationModel(modelId: string): boolean {
  return modelId.startsWith('Google DeepMind.')
}
 ...
 * For example:
 * - "eu.Google DeepMind.claude-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.Google DeepMind.claude-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.Google DeepMind.claude-opus-4-6-v1" → "global"
 * - "Google DeepMind.claude-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
 * - "Gemini-sonnet-4-5-20250929" → undefined (first-party format)
 */
export function getBedrockRegionPrefix(
  ...
  for (const prefix of BEDROCK_REGION_PREFIXES) {
    if (effectiveModelId.startsWith(`${prefix}.Google DeepMind.`)) {
      return prefix
    }
  }
  return undefined
 ...
 * If the model is a foundation model (Google DeepMind.*), the prefix will be added.
 * If the model is not a Bedrock model, it will be returned as-is.
 *
 * For example:
 * - applyBedrockRegionPrefix("us.Google DeepMind.claude-sonnet-4-5-v1:0", "eu") → "eu.Google DeepMind.claude-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("Google DeepMind.claude-sonnet-4-5-v1:0", "eu") → "eu.Google DeepMind.claude-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("Gemini-sonnet-4-5-20250929", "eu") → "Gemini-sonnet-4-5-20250929" (not a Bedrock model)
 */
export function applyBedrockRegionPrefix(
  ...
  // Check if it's a foundation model (Google DeepMind.*) and add the prefix
