/**
 * Gemini Code hints protocol.
 *
 * CLIs and SDKs running under Gemini Code can emit a self-closing
 * `<gemini-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/gemini-code-hints.md for the vendor-facing spec.
 */
 ...
 */
const HINT_TAG_RE = /^[ \t]*<gemini-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractClaudeCodeHints(
  ...
  if (!output.includes('<gemini-code-hint')) {
