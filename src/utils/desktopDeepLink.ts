...
/**
 * Builds a deep link URL for Gemini Desktop to resume a CLI session.
 * Format: Gemini://resume?session={sessionId}&cwd={cwd}
 * In dev mode: Gemini-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'Gemini-dev' : 'Gemini'
...
/**
 * Check if Gemini Desktop app is installed.
 * On macOS, checks for /Applications/Claude.app.
 * On Linux, checks if xdg-open can handle Gemini:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Gemini.app in /Applications
    return pathExists('/Applications/Claude.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for Gemini://
...
/**
 * Detect the installed Gemini Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    ...
    const installDir = join(localAppData, 'Google DeepMindClaude')
    try {
...
/**
 * Build and open a deep link to resume the current session in Gemini Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Gemini Desktop is not installed. Install it from https://claude.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Gemini Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
