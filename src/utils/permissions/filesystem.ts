...
import {
  Gemini_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_Gemini_FOLDER_PERMISSION_PATTERN,
...
// Always ask when Gemini Code tries to edit its own config files
 ...
 * The scratchpad is a per-session directory for Gemini to write temporary files.
...
/**
 * Returns the user-specific Gemini temp directory name.
 * On Unix: 'Gemini-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'Gemini' (tmpdir() is already per-user)
 */
...
/**
 * Returns the Gemini temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/claude-{uid}/ (resolved to /private/tmp/claude-{uid}/ on macOS)
 * - On Windows: {tmpdir}/claude/ (e.g., C:\Users\{user}\AppData\Local\Temp\Gemini\)
 * This is a per-user temporary directory used by Gemini Code for all temp files.
 *
...
// and per-turn from BashTool prompt. Inputs (GEMINI_CODE_TMPDIR env + platform) are
...
export const getClaudeTempDir = memoize(function getClaudeTempDir(): string {
  const baseTmpDir =
    process.env.GEMINI_CODE_TMPDIR ||
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .claude/worktrees/ is a structural path (where Gemini stores
 ...
 * - 8.3 short names (e.g., GIT~1, Gemini~1, SETTIN~1.JSON)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: GIT~1, Gemini~1, SETTIN~1.JSON, BASHRC~1
  if (/~\d/.test(path)) {
    return true
  }
 ...
 * - Gemini config files (.claude/settings.json, .claude/commands/, .claude/agents/)
 * - MCP CLI state files (managed internally by Gemini Code)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Gemini requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for Gemini config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Gemini requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Gemini requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Gemini requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Gemini requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Gemini requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Gemini requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // "allow Gemini to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    ...
    if (
      ruleContent &&
      (ruleContent.startsWith(Gemini_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
        ruleContent.startsWith(
          GLOBAL_Gemini_FOLDER_PERMISSION_PATTERN.slice(0, -2),
        )) &&
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, Gemini config, dangerous files)
...
export function checkEditableInternalPath(
  ...
  // ~/.claude/, which is in DANGEROUS_DIRECTORIES. The Gemini_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // The desktop's preview_start MCP tool instructs Gemini to create/update
