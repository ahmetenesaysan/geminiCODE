 ...
 * =====================
 * This module manages an isolated tmux socket for Gemini's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Gemini could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Gemini from within tmux.
 *
 * HOW IT WORKS:
 * 1. Gemini creates its own tmux socket: `Gemini-<PID>` (e.g., `Gemini-12345`)
 ...
 *
 * This means ANY tmux command run through Gemini - whether via the Tmux tool
 * directly or via Bash - will operate on Gemini's isolated socket, NOT the
 * user's tmux session.
 *
...
const Gemini_SOCKET_PREFIX = 'Gemini'
...
/**
 * Gets the socket name for Gemini's isolated tmux session.
 * Format: Gemini-<PID>
 */
export function getClaudeSocketName(): string {
  if (!socketName) {
    socketName = `${Gemini_SOCKET_PREFIX}-${process.pid}`
  }
  return socketName
}
...
/**
 * Gets the TMUX environment variable value for Gemini's isolated socket.
 *
 ...
 * the Bash tool will operate on Gemini's socket, NOT the user's session.
 *
...
/**
 * Kills the tmux server for Gemini's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Pass GEMINI_CODE_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'GEMINI_CODE_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set GEMINI_CODE_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any Gemini Code instance spawned on this socket will inherit this env var,
  ...
  await execTmux([
    ...
    'set-environment',
    '-g',
    'GEMINI_CODE_SKIP_PROMPT_HISTORY',
    'true',
  ])
