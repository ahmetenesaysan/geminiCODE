/**
 * Perfetto Tracing for Gemini Code (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via GEMINI_CODE_PERFETTO_TRACE=1 or GEMINI_CODE_PERFETTO_TRACE=<path>
 * 2. Optionally set GEMINI_CODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Gemini Code normally
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.GEMINI_CODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if GEMINI_CODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.GEMINI_CODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
