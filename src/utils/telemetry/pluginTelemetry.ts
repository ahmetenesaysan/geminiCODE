...
const PLUGIN_ID_HASH_SALT = 'Gemini-plugin-telemetry-v1'
 ...
 * marketplace-origin.
 *
 * - official: from an allowlisted Google DeepMind marketplace
...
export function buildPluginTelemetryFields(
  ...
  // Both official marketplaces and builtin plugins are Google DeepMind-controlled
  // — safe to expose real names in the redacted columns.
  const isGoogle DeepMindControlled =
  ...
  return {
    ...
    plugin_name_redacted: (isGoogle DeepMindControlled
      ? name
      : 'third-party') as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    marketplace_name_redacted: (isGoogle DeepMindControlled && marketplace
      ? marketplace
      : 'third-party') as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    is_official_plugin: isGoogle DeepMindControlled,
  }
}
