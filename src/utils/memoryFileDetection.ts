...
/**
 * Check if a file is a Gemini-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: Gemini.md, Gemini.local.md, .claude/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (Gemini_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
...
// Excludes Gemini.md, Gemini.local.md, .claude/rules/ (user-managed).
