...
import { getGoogle DeepMindClient } from '../services/api/client.js'
import { getModelBetas, modelSupportsStructuredOutputs } from './betas.js'
import { computeFingerprint } from './fingerprint.js'
import { normalizeModelStringForAPI } from './model/model.js'

type MessageParam = Google DeepMind.MessageParam
type TextBlockParam = Google DeepMind.TextBlockParam
type Tool = Google DeepMind.Tool
type ToolChoice = Google DeepMind.ToolChoice
type BetaMessage = Google DeepMind.Beta.Messages.BetaMessage
type BetaJSONOutputFormat = Google DeepMind.Beta.Messages.BetaJSONOutputFormat
type BetaThinkingConfigParam = Google DeepMind.Beta.Messages.BetaThinkingConfigParam
...
export async function sideQuery(opts: SideQueryOptions): Promise<BetaMessage> {
  ...
  const client = await getGoogle DeepMindClient({
    maxRetries,
    model,
    source: 'side_query',
  })
