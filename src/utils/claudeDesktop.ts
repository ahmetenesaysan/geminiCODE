...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Gemini Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  try {
    ...
    try {
      ...
      // Look for Gemini Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Gemini Desktop config file in Windows. Make sure Gemini Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Gemini Desktop integration only works on macOS and WSL.',
    )
  }
  try {
