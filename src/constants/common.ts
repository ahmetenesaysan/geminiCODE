...
export function getLocalISODate(): string {
  ...
  if (process.env.GEMINI_CODE_OVERRIDE_DATE) {
    return process.env.GEMINI_CODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.GEMINI_CODE_OVERRIDE_DATE
    ? new Date(process.env.GEMINI_CODE_OVERRIDE_DATE)
    : new Date()
