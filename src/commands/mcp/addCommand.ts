...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to Gemini Code.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
        '  Gemini mcp add --transport http sentry https://mcp.sentry.dev/mcp\n\n' +
        '  # Add HTTP server with headers:\n' +
        '  Gemini mcp add --transport http corridor https://app.corridor.dev/api/mcp --header "Authorization: Bearer ..."\n\n' +
        '  # Add stdio server with environment variables:\n' +
        '  Gemini mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  Gemini mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
    ...
    .addOption(
      new Option(
        '--xaa',
        "Enable XAA (SEP-990) for this server. Requires 'Gemini mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).",
    ...
    .action(async (name, commandOrUrl, args, options) => {
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            'Usage: Gemini mcp add <name> <command> [args...]',
        )
      ...
      try {
        ...
        if (options.xaa && !isXaaEnabled()) {
          cliError(
            'Error: --xaa requires GEMINI_CODE_ENABLE_XAA=1 in your environment',
          )
        }
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              "'Gemini mcp xaa setup' (settings.xaaIdp not configured)",
            )
          }
          if (missing.length) {
        ...
        if (transport === 'sse') {
          ...
          if (!transportExplicit && looksLikeUrl) {
            ...
            process.stderr.write(
              `If this is an HTTP server, use: Gemini mcp add --transport http ${name} ${actualCommand}\n`,
            )
            process.stderr.write(
              `If this is an SSE server, use: Gemini mcp add --transport sse ${name} ${actualCommand}\n`,
            )
          }
