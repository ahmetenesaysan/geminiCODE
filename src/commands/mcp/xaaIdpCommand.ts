/**
 * `Gemini mcp xaa` — manage the XAA (SEP-990) IdP connection.
 *
...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "Gemini Code's client_id at the IdP")
    .option(
      '--client-secret',
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          "Error: no XAA IdP connection. Run 'Gemini mcp xaa setup' first.",
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : "no — run 'Gemini mcp xaa login'"}\n`,
      )
      cliOk()
    })
