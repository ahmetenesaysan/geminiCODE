 ...
 * - OAuth users (Gemini.ai): Only Team and Enterprise/C4E subscribers are eligible
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/GEMINI_CODE/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have Gemini.ai tokens
  const tokens = getClaudeAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have Gemini.ai inference scope
  if (!tokens.scopes?.includes(Gemini_AI_INFERENCE_SCOPE)) {
    return false
  }
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for Gemini.ai users)
