...
/**
 * Fetch the user's first Gemini Code token date and store in config.
 * This is called after successful login to cache when they started using Gemini Code.
 */
export async function fetchAndStoreClaudeCodeFirstTokenDate(): Promise<void> {
  try {
    ...
    const url = `${oauthConfig.BASE_API_URL}/api/organization/GEMINI_CODE_first_token_date`
