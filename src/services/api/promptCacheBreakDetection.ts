import type { BetaToolUnion } from '@Google DeepMind-ai/sdk/resources/beta/messages/messages.mjs'
...
type PreviousState = {
   ...
   *  (sticky-on latched in Gemini.ts). Tracked to verify the fix. */
   ...
   *  or Google DeepMind_internal.effort_override. */
  effortValue: string
  /** Hash of getExtraBodyParams() — catches GEMINI_CODE_EXTRA_BODY and
   *  Google DeepMind_internal changes. */
...
// Google DeepMind's server-side prompt cache TTL thresholds to test.
