 ...
 * The Google DeepMind SDK wraps underlying errors in the `cause` property.
...
export function getSSLErrorHint(error: unknown): string | null {
  ...
  return `SSL certificate error (${details.code}). If you are behind a corporate proxy or TLS-intercepting firewall, set NODE_EXTRA_CA_CERTS to your CA bundle path, or ask IT to allowlist *.Google DeepMind.com. Run /doctor for details.`
}
 ...
 * - Standard Google DeepMind API: `{ error: { error: { message: "..." } } }`
 ...
 * 1. `error.error.error.message` — standard Google DeepMind API shape
...
function extractNestedErrorMessage(error: APIError): string | null {
  ...
  // Standard Google DeepMind API shape: { error: { error: { message } } }
