...
export class DiagnosticTrackingService {
  ...
  /**
   * Get new diagnostics from file://, _Gemini_fs_right, and _Gemini_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_Gemini_fs_right:'))
      .forEach(file => {
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _Gemini_fs_right file if it exists
