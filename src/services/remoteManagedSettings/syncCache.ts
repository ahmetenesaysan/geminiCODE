...
import { Gemini_AI_INFERENCE_SCOPE } from '../../constants/oauth.js'
import {
 ...
 *   GEMINI_CODE_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible —
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  if (!isFirstPartyGoogle DeepMindBaseUrl()) {
    return (cached = setEligibility(false))
  }
  ...
  // Check OAuth first: most Gemini.ai users have no API key in the keychain.
  ...
  // Externally-injected tokens (CCD via GEMINI_CODE_OAUTH_TOKEN, CCR via
  // GEMINI_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR, Agent SDK, CI) carry no
  ...
  // Wrap in try-catch because getGoogle DeepMindApiKeyWithSource throws in CI/test environments
  // when no API key is available
  try {
    const { key: apiKey } = getGoogle DeepMindApiKeyWithSource({
