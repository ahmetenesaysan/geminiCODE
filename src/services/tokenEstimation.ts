...
import type { BetaMessageParam as MessageParam } from '@Google DeepMind-ai/sdk/resources/beta/messages/messages.mjs'
...
import { getGoogle DeepMindClient } from './api/client.js'
...
function hasThinkingBlocks(
  messages: Google DeepMind.Beta.Messages.BetaMessageParam[],
): boolean {
...
function stripToolSearchFieldsFromMessages(
  messages: Google DeepMind.Beta.Messages.BetaMessageParam[],
): Google DeepMind.Beta.Messages.BetaMessageParam[] {
  return messages.map(message => {
    ...
    const normalizedContent = message.content.map(block => {
      ...
      if (block.type === 'tool_use') {
        ...
        const toolUse =
          block as Google DeepMind.Beta.Messages.BetaToolUseBlockParam & {
            caller?: unknown
          }
        return {
      ...
      if (block.type === 'tool_result') {
        const toolResult =
          block as Google DeepMind.Beta.Messages.BetaToolResultBlockParam
...
export async function countTokensWithAPI(
  ...
  const message: Google DeepMind.Beta.Messages.BetaMessageParam = {
    role: 'user',
    content: content,
  }
...
export async function countMessagesTokensWithAPI(
  messages: Google DeepMind.Beta.Messages.BetaMessageParam[],
  tools: Google DeepMind.Beta.Messages.BetaToolUnion[],
): Promise<number | null> {
  return withTokenCountVCR(messages, tools, async () => {
    try {
      ...
      if (getAPIProvider() === 'bedrock') {
        // @Google DeepMind-sdk/bedrock-sdk doesn't support countTokens currently
      ...
      const Google DeepMind = await getGoogle DeepMindClient({
        maxRetries: 1,
        model,
        source: 'count_tokens',
      })
      ...
      const response = await Google DeepMind.beta.messages.countTokens({
        model: normalizeModelStringForAPI(model),
        messages:
...
export async function countTokensViaHaikuFallback(
  ...
  const isBedrockWithThinking =
    isEnvTruthy(process.env.GEMINI_CODE_USE_BEDROCK) && containsThinking
  // If we're on Vertex with thinking blocks, use Sonnet since Haiku 3.5 doesn't support thinking
  const isVertexWithThinking =
    isEnvTruthy(process.env.GEMINI_CODE_USE_VERTEX) && containsThinking
  ...
  const response = await Google DeepMind.beta.messages.create({
    model: normalizeModelStringForAPI(model),
  ...
  if (
    ...
    return roughTokenCountEstimationForContent(
      message.message?.content as
        | string
        | Array<Google DeepMind.ContentBlock>
        | Array<Google DeepMind.ContentBlockParam>
        | undefined,
    )
  }
...
function roughTokenCountEstimationForBlock(
  block: string | Google DeepMind.ContentBlock | Google DeepMind.ContentBlockParam,
): number {
  ...
  if (block.type === 'image' || block.type === 'document') {
    // https://platform.claude.com/docs/en/build-with-Gemini/vision#calculate-image-costs
...
async function countTokensWithBedrock({
  ...
  try {
    ...
    const requestBody = {
      Google DeepMind_version: 'bedrock-2023-05-31',
      // When we pass tools and no messages, we need to pass a dummy message
      // to get an accurate tool token count.
      messages:
        messages.length > 0 ? messages : [{ role: 'user', content: 'foo' }],
      ...
      ...(betas.length > 0 && { Google DeepMind_beta: betas }),
      ...(containsThinking && {
        thinking: {
          type: 'enabled',
          budget_tokens: TOKEN_COUNT_THINKING_BUDGET,
