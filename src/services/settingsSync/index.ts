 ...
 *
 * Syncs user settings and memory files across Gemini Code environments.
 *
 ...
 *
 * Backend API: Google DeepMind/Google DeepMind#218817
 */
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken && tokens.scopes?.includes(Gemini_AI_INFERENCE_SCOPE),
  )
}

function getSettingsSyncEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/GEMINI_CODE/user_settings`
}
 ...
 * - clearMemoryFileCaches() for memory files (Gemini.md)
 */
