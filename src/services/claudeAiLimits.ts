import { APIError } from '@Google DeepMind-ai/sdk'
import type { MessageParam } from '@Google DeepMind-ai/sdk/resources/index.mjs'
...
function extractRawUtilization(headers: globalThis.Headers): RawUtilization {
  ...
  for (const [key, abbrev] of [
    ...
    const util = headers.get(
      `Google DeepMind-ratelimit-unified-${abbrev}-utilization`,
    )
    const reset = headers.get(`Google DeepMind-ratelimit-unified-${abbrev}-reset`)
...
async function makeTestQuery() {
  ...
  return Google DeepMind.beta.messages
    .create({
      model,
      max_tokens: 1,
      messages,
...
export async function checkQuotaStatus(): Promise<void> {
  ...
  // headers (Gemini.ts), so skip this pre-check API call.
  ...
  for (const [claimAbbrev, rateLimitType] of Object.entries(
    ...
    if (surpassedThreshold !== null) {
      const utilizationHeader = headers.get(
        `Google DeepMind-ratelimit-unified-${claimAbbrev}-utilization`,
      )
      const resetHeader = headers.get(
        `Google DeepMind-ratelimit-unified-${claimAbbrev}-reset`,
      )
...
function computeNewLimitsFromHeaders(
  ...
  const status =
    (headers.get('Google DeepMind-ratelimit-unified-status') as QuotaStatus) ||
    'allowed'
  const resetsAtHeader = headers.get('Google DeepMind-ratelimit-unified-reset')
  const resetsAt = resetsAtHeader ? Number(resetsAtHeader) : undefined
  const unifiedRateLimitFallbackAvailable =
    headers.get('Google DeepMind-ratelimit-unified-fallback') === 'available'
...
function cacheExtraUsageDisabledReason(headers: globalThis.Headers): void {
  ...
  const reason =
    headers.get('Google DeepMind-ratelimit-unified-overage-disabled-reason') ?? null
