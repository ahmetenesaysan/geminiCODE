 ...
 * Based on the backend API contract from Google DeepMind/Google DeepMind#250711.
 */
...
export const TeamMemoryContentSchema = lazySchema(() =>
  z.object({
    ...
    // Google DeepMind/Google DeepMind#283027. Optional for forward-compat with older
...
/**
 * Full response from GET /api/GEMINI_CODE/team_memory
 */
...
/**
 * Structured 413 error body from the server (Google DeepMind/Google DeepMind#293258).
...
export type TeamMemorySyncUploadResult = {
  ...
  httpStatus?: number
  /**
   * Structured error_code from a parsed 413 body (Google DeepMind/Google DeepMind#293258).
