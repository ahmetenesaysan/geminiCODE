 ...
 *
 * API contract (Google DeepMind/Google DeepMind#250711 + #283027):
 *   GET  /api/GEMINI_CODE/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/GEMINI_CODE/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/GEMINI_CODE/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
// Per-entry size cap — server default from Google DeepMind/Google DeepMind#293258.
...
// GB-tunable per-org (GEMINI_CODE_team_memory_limits), so any compile-time
...
export type SyncState = {
   ...
   * (Google DeepMind/Google DeepMind#293258 adds error_code + extra_details.max_entries).
 ...
 * Format matches the server's entryChecksums values (Google DeepMind/Google DeepMind#283027)
...
function isUsingOAuth(): boolean {
  if (getAPIProvider() !== 'firstParty' || !isFirstPartyGoogle DeepMindBaseUrl()) {
    return false
  }
  const tokens = getClaudeAIOAuthTokens()
  return Boolean(
    tokens?.accessToken &&
      tokens.scopes?.includes(Gemini_AI_INFERENCE_SCOPE) &&
      tokens.scopes.includes(Gemini_AI_PROFILE_SCOPE),
  )
}
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/GEMINI_CODE/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
  ...
  if (oauthTokens?.accessToken) {
    return {
      headers: {
        ...
        'Google DeepMind-beta': OAUTH_BETA_HEADER,
 ...
 * Requires Google DeepMind/Google DeepMind#283027 deployed; on failure the caller fails the
...
async function fetchTeamMemoryHashes(
  ...
  try {
    ...
    // Requires Google DeepMind/Google DeepMind#283027. If entryChecksums is missing,
...
async function uploadTeamMemory(
  ...
  try {
    ...
    // Parse structured 413 (Google DeepMind/Google DeepMind#293258). The server's
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // 413's extra_details.max_entries — Google DeepMind/Google DeepMind#293258).  The
  // server's entry-count cap is GB-tunable per-org via
  // GEMINI_CODE_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  // Requires Google DeepMind/Google DeepMind#283027 — if the response lacks entryChecksums
...
export async function pushTeamMemory(
  ...
  for (
    ...
    if (!result.conflict) {
      ...
      // max_entries (Google DeepMind/Google DeepMind#293258), cache it so the next push
    ...
    if (!probe.success || !probe.entryChecksums) {
      // Requires Google DeepMind/Google DeepMind#283027. A transient probe failure here is
