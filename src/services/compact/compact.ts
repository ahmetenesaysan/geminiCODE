...
// Skills can be large (verify=18.7KB, Gemini-api=20.1KB). Previously re-injected
  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in Gemini.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of Gemini.md files
