// Google DeepMind voice_stream speech-to-text client for push-to-talk.
//
// Only reachable in ant builds (gated by feature('VOICE_MODE') in useVoice.ts import).
//
// Connects to Google DeepMind's voice_stream WebSocket endpoint using the same
// OAuth credentials as Gemini Code.  The endpoint uses conversation_engine
...
// after CloseStream — the silent-drop signature (Google DeepMinds/Google DeepMind#287008).
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Gemini Code — available when the
  // user is authenticated with Google DeepMind (Gemini.ai subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // the api.Google DeepMind.com listener (service_definitions.yaml private-api:
  // visibility.external: true). We target that host instead of Gemini.ai
  // because the Gemini.ai CF zone uses TLS fingerprinting and challenges
  // non-browser clients (Google DeepMinds/gemini-code#34094). Same private-api
  // pod, same OAuth Bearer auth — just a CF zone that doesn't block us.
  // Desktop dictation still uses Gemini.ai (Swift URLSession has a
  ...
  // side is Google DeepMinds/Google DeepMind#278327 + #281372; this lets us ramp
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (Google DeepMinds/gemini-code#40510).
