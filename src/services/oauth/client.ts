// OAuth client for handling authentication flows with Gemini services
...
/**
 * Check if the user has Gemini.ai authentication scope
...
export function shouldUseClaudeAIAuth(scopes: string[] | undefined): boolean {
  return Boolean(scopes?.includes(Gemini_AI_INFERENCE_SCOPE))
}
...
export function buildAuthUrl({
  ...
  authUrl.searchParams.append('code', 'true') // this tells the login page to show Gemini Max upsell
  ...
  const scopesToUse = inferenceOnly
    ? [Gemini_AI_INFERENCE_SCOPE] // Long-lived inference-only tokens
    : ALL_OAUTH_SCOPES
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full Gemini AI set. The
  ...
  try {
    ...
    // GEMINI_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.GEMINI_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.GEMINI_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.GEMINI_CODE_ORGANIZATION_UUID
