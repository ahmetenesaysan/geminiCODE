 ...
 * capabilities.experimental['Gemini/channel/permission'].
 *
 * Kenneth's "would this let Gemini self-approve?": the approving party is
 * the human via the channel, not Gemini. But the trust boundary isn't the
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['Gemini/channel'] !== undefined &&
      c.capabilities?.experimental?.['Gemini/channel/permission'] !== undefined,
  )
}
