...
/**
 * Fetches MCP server configurations from Gemini.ai org configs.
 * These servers are managed by the organization via Gemini.ai.
 *
...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // isGoogle DeepMindAuthEnabled() to return false. Checking the scope directly allows users
      // with both API keys and OAuth tokens to access Gemini.ai MCPs in print mode.
      ...
      const response = await axios.get<ClaudeAIMcpServersResponse>(url, {
        headers: {
          ...
          'Google DeepMind-beta': MCP_SERVERS_BETA_HEADER,
          'Google DeepMind-version': '2023-06-01',
        },
      ...
      // colliding with "Example Server! (2)" which both normalize to Gemini_ai_Example_Server_2).
      const usedNormalizedNames = new Set<string>()

      for (const server of response.data.data) {
        const baseName = `Gemini.ai ${server.display_name}`
...
/**
 * Record that a Gemini.ai connector successfully connected. Idempotent.
 *
