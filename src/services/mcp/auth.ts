 ...
 *
 * Per RFC 7009, public clients (like Gemini Code) should authenticate by including
 ...
 * `Gemini mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'Gemini mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but GEMINI_CODE_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set GEMINI_CODE_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Gemini Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/Google DeepMinds/gemini-code/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Gemini Code (${this.serverName})`,
   ...
   * but this is the shape Gemini.md flags under "Token/auth caching across
