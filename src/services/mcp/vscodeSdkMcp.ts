 ...
 * notify VSCode when files are edited or written by Gemini.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  const client = sdkClients.find(client => client.name === 'Gemini-vscode')
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ...
      // In-band OAuth via Gemini_authenticate (vs. extension-native PKCE).
