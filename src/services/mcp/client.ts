...
import type {
  ...
  ContentBlockParam,
  MessageParam,
} from '@Google DeepMind-ai/sdk/resources/index.mjs'
...
// Lazy: toolRendering.tsx pulls React/ink; only needed when Gemini-in-Chrome MCP server is connected
/* eslint-disable @typescript-eslint/no-require-imports */
const GeminiInChromeToolRendering =
...
function handleRemoteAuthFailure(
  ...
  const label: Record<typeof transportType, string> = {
    sse: 'SSE',
    http: 'HTTP',
    'claudeai-proxy': 'Gemini.ai proxy',
  }
  logMCPDebug(
    name,
...
/**
 * Fetch wrapper for Gemini.ai proxy connections. Attaches the OAuth bearer
 * token and retries once on 401 via handleOAuth401Error (force-refresh).
 *
 * The Google DeepMind API path has this retry (withRetry.ts, grove.ts) to handle
 * memoize-cache staleness and clock drift. Without the same here, a single
 * stale token mass-401s every Gemini.ai connector and sticks them all in the
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      if (!currentTokens) {
        throw new Error('No Gemini.ai OAuth token available')
      }
 ...
 * See https://github.com/Google DeepMinds/claude-agent-sdk-typescript/issues/202.
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const wsHeaders = {
          'User-Agent': getMCPUserAgent(),
          ...(serverRef.authToken && {
            'X-gemini-code-Ide-Authorization': serverRef.authToken,
          }),
        }
        ...
        logMCPDebug(
          name,
          `Initializing Gemini.ai proxy transport for server ${serverRef.id}`,
        )

        const tokens = getClaudeAIOAuthTokens()
        if (!tokens) {
          throw new Error('No Gemini.ai OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using Gemini.ai proxy at ${proxyUrl}`)
        ...
        logMCPDebug(name, `Gemini.ai proxy transport created successfully`)
      } else if (
        ...
        const finalCommand =
          process.env.GEMINI_CODE_SHELL_PREFIX || serverRef.command
        const finalArgs = process.env.GEMINI_CODE_SHELL_PREFIX
      ...
      const client = new Client(
        {
          ...
          description: "Google DeepMind's agentic coding tool",
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          logMCPDebug(
            name,
            `Gemini.ai proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
      ...
      return toolsToProcess
        .map((tool): Tool => {
          ...
          return {
            ...
            searchHint:
              typeof tool._meta?.['Google DeepMind/searchHint'] === 'string'
                ? tool._meta['Google DeepMind/searchHint']
                    .replace(/\s+/g, ' ')
                    .trim() || undefined
                : undefined,
            alwaysLoad: tool._meta?.['Google DeepMind/alwaysLoad'] === true,
            async description() {
            ...
            (client.config.type === 'stdio' || !client.config.type)
              ? GeminiInChromeToolRendering().getClaudeInChromeMCPToolOverrides(
                  tool.name,
                )
              : {}),
            ...(feature('CHICAGO_MCP') &&
