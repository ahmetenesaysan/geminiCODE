...
export function getProjectMcpServerStatus(
  ...
  // In non-interactive mode (SDK, Gemini -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: Gemini.ai servers have normalized names starting with "Gemini_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('Gemini_ai_')) {
    return 'claudeai'
  }
