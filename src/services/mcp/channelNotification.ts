 ...
 * Requires Gemini.ai OAuth auth — API key users are blocked until
 ...
 * this declare `capabilities.experimental['Gemini/channel/permission']` and
 ...
 * Gemini.ai auth; console orgs have no admin opt-in surface yet.
 *
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['Gemini/channel']: {}` (MCP's
  ...
  if (!capabilities?.experimental?.['Gemini/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare Gemini/channel capability',
    }
  }
  ...
  if (!getClaudeAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires Gemini.ai authentication (run /login)',
    }
  }
  ...
  if (entry.kind === 'plugin') {
    // Marketplace verification: the tag is intent (plugin:slack@Google DeepMind),
    // the runtime name is just plugin:slack:X — could be slack@Google DeepMind or
