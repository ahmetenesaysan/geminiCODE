...
/**
 * CCR proxy URL path markers. In remote sessions, Gemini.ai connectors arrive
 ...
 * plugin deduplication. Ignores env (plugins always inject Gemini_PLUGIN_ROOT)
...
/**
 * Filter Gemini.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `Gemini mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `Gemini.ai <DisplayName>` so they never key-collide with
 ...
 * `mcp__Gemini_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing Gemini.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
export async function addMcpConfig(
  ...
  // Block reserved server name "Gemini-in-chrome"
...
/**
 * Get Gemini Code MCP configurations (excludes Gemini.ai servers from the
 ...
 * Gemini.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Gemini Code server configurations with appropriate scopes
 */
...
/**
 * Get all MCP configurations across all scopes, including Gemini.ai servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load Gemini.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getClaudeCodeMcpConfigs()
  }

  // Kick off the Gemini.ai fetch before getClaudeCodeMcpConfigs so it overlaps
  ...
  // Suppress Gemini.ai connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `Gemini.ai Slack`) so the merge below
  ...
  // Merge with Gemini.ai having lowest precedence
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // what the best way to do this is. In the meantime, we are limiting this to Gemini-vscode for now to
  // unbreak the VSCode extension for certain enterprise customers who have enterprise MCP config enabled.
  // https://Google DeepMind.slack.com/archives/C093UA0KLD7/p1764975463670109
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'Gemini-vscode',
  )
}
