...
/**
 * Map LSP severity to Gemini diagnostic severity
 *
 * Maps LSP severity numbers to Gemini diagnostic severity strings.
...
/**
 * Convert LSP diagnostics to Gemini diagnostic format
 *
 * Converts LSP PublishDiagnosticsParams to DiagnosticFile[] format
 * used by Gemini's attachment system.
 */
 ...
 * from all LSP servers and routes them to Gemini's diagnostic system.
      ...
      serverInstance.onNotification(
        ...
        (params: unknown) => {
          ...
          try {
            ...
            // Convert LSP diagnostics to Gemini format (can throw on invalid URIs)
